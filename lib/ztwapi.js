...
StreamHandler.prototype.start = function(){
  ...
  var request = this.oa.get(url, access_token, access_token_secret, null);
  ...
  request.on('response', function(response) {
    ...
    response.on('data', function(data){
      ...
      if( endofobject == -1) {
        ...
        //console.log("sending :"+objectbuffer +":to parser" )
        if( objectbuffer !== '\r\n')
          twitter_data = JSON.parse(objectbuffer);
        else
          console.log("keep alive...")

        if( twitter_data && twitter_data.user )
          console.log(twitter_data.user.name + ": " +
              twitter_data.text);
  ...
  console.log("StreamHandler started")
